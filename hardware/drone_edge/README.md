# Orca Drone Edge Stack

This folder is the non-container drone-side runtime for Orca.
...
- IMU, GPS, barometer, and magnetometer telemetry packaged into Orca payloads
- RGB, thermal, and zoom camera stream registration for RTSP or WebRTC uplink
- 4G, 5G, or WiFi transport into the Orca cloud services
- a custom Orca drone SDK for telemetry, sensor, and video registration
...
- `sdk.py`: Orca cloud uplink SDK for Drone Gateway, Sensor Gateway, and Camera Service.
...
- `manufacturer_spec.py`: manufacturer-facing hardware specification contract for Orca-compatible drones.
- `ros2_contract.py`: ROS2 autonomy-node contract for SLAM, obstacle avoidance, and visual odometry publishing into Orca.
...
The drone companion computer owns all communication from drone hardware into ORCA:

1. PX4 publishes flight telemetry over MAVLink.
2. ROS2 autonomy nodes provide local perception and navigation status.
3. `mavlink_bridge.py` converts those payloads into Orca telemetry and capability records.
4. `OrcaDroneSDK` registers the drone with Drone Gateway.
...
describes the node responsibilities, ROS topics, and Orca publish contract
...
Orca-controlled encoder contract for H.264 or H.265 plus RTSP, WebRTC, or
...
sdk = OrcaDroneSDK(
    drone_gateway_base_url="http://gateway.orca.local/drone-gateway",
    sensor_gateway_base_url="http://gateway.orca.local/sensor-gateway",
    camera_service_base_url="http://gateway.orca.local/drone-camera",
)
...
This repo does not build PX4 firmware or ROS2 packages directly. Instead, it implements the Orca-owned integration layer between those drone runtimes and the Orca cloud platform.
