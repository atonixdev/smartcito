# SmartCito Drone Manufacturer Specification And RFP Packet

This document is the manufacturer-facing hardware specification and RFP packet structure for a SmartCito-compatible drone platform.

The delivery model is explicit: the manufacturer builds the airframe and electronics, while the SmartCito team owns the companion-computer software, MAVLink integration, camera behavior, ROS2 autonomy software, and the SmartCito cloud services.

## Cover Page

- Document: SmartCito Drone Platform RFP Packet
- Program: SmartCito drone airframe and electronics procurement
- Delivery model: manufacturer builds body and electronics; SmartCito owns software and cloud
- Response requested: prototype and pilot-batch proposal

## Executive Summary

SmartCito is procuring a drone platform that supports PX4 or ArduPilot,
MAVLink, a SmartCito-controlled companion computer, RGB or thermal or zoom
...
SmartCito retains control of the companion-computer software stack, including
ROS2 autonomy nodes, the SmartCito Drone SDK, camera streaming behavior, and
...
- 4G, 5G, or WiFi connectivity from the companion computer to the SmartCito cloud
- MAVLink telemetry and mission upload available from the companion computer
- RTSP, WebRTC, or SmartCito-approved custom video uplink from the companion computer

High-level path:

1. Camera connects to companion computer.
2. Companion computer encodes video as H.264 or H.265.
3. Companion computer streams video to SmartCito Camera Service.
4. Camera Service runs AI and threat detection.
5. Results flow to the dashboard through the SmartCito cloud.

## 1A. ROS2 Autonomy Node Contract

The companion computer must support SmartCito-owned ROS2 nodes for:

- SLAM
- obstacle avoidance
- visual odometry

These nodes must be installable and operable by the SmartCito team. Required
...
These nodes publish into the SmartCito edge runtime and ultimately feed:
...
The companion computer is the SmartCito-owned software surface.
...
- RAM and storage sized for:
  ...
  - SmartCito SDK and ROS2 packages
- Physical interfaces:
  - Ethernet
  - USB
  - UART
...
The SmartCito team must be able to:
...
No locked-down vendor image is acceptable unless full SmartCito software installation rights remain intact.
...
- video uplink from companion computer to SmartCito Camera Service
- secure IP connectivity from companion computer to SmartCito cloud
...
3. Keep software and cloud ownership with the SmartCito team.
4. Validate the delivered platform against PX4/ArduPilot, MAVLink, SSH access, camera accessibility, and SmartCito cloud integration.

## 9. SmartCito Acceptance Criteria

The platform is acceptable only if the SmartCito team can:
...
- register the drone and camera into SmartCito cloud services without vendor dependency
...
- SmartCito team can SSH into the companion computer
- SmartCito team can install its own SDK and ROS2 nodes
- camera is accessible for frame capture and H.264 or H.265 encoding
- RTSP or WebRTC uplink works from companion computer to SmartCito Camera Service
...
1. Can you build the airframe and electronics to this specification while preserving SmartCito software control?
