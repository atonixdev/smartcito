...
@dataclass(slots=True)
class SmartCitoTrainingMetrics:
    version: str
    records: int
...
class SmartCitoModel:
    ...
    @classmethod
    def load(cls, model_dir: str | Path) -> SmartCitoModel:
        ...
        return cls(
            ...
            metrics=SmartCitoTrainingMetrics(**dict(payload.get("metrics") or {})),
        )
...
def next_model_version(models_dir: str | Path = DEFAULT_MODELS_DIR) -> str:
    ...
    existing = sorted((path for path in directory.glob("smartcito_model_v*") if path.is_dir()), key=_version_sort_key)
    next_version = _version_sort_key(existing[-1]) + 1 if existing else 1
    return f"smartcito_model_v{next_version}"
...
def load_active_model(*, models_dir: str | Path = DEFAULT_MODELS_DIR) -> SmartCitoModel:
    pointer = Path(models_dir) / ACTIVE_MODEL_POINTER.name
    payload = json.loads(pointer.read_text(encoding="utf-8"))
    return SmartCitoModel.load(payload["active_model_dir"])
