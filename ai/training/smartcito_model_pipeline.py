...
from ai.smartcito_runtime.model import (
    DEFAULT_MODELS_DIR,
...
DEFAULT_BATCH_DIR = Path("ai/smartcito_datasets")


def load_training_batches(batch_dir: str | Path = DEFAULT_BATCH_DIR) -> tuple[list[SmartCitoTrainingRecord], list[Path]]:
    ...
    for path in sorted(directory.glob("batch_*.json")):
        ...
        for item in payload:
            if not isinstance(item, dict):
                continue
            records.append(SmartCitoTrainingRecord(**item))
        sources.append(path)
    return records, sources


def _fit_metrics(model_records: list[SmartCitoTrainingRecord]) -> dict[str, Any]:
