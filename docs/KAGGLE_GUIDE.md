...
```bash
python training/package_kaggle_bundle.py
```
...
That folder already contains the SmartCito AI code, training scripts, sample datasets, notebooks, and contributor docs needed for Kaggle.
...
3. Install dependencies from `ai_models/requirements.txt`.
...
```python
!pip install -r /kaggle/input/smartcito/ai_models/requirements.txt
```

## Training Steps

1. Open [examples/smartcito_training_demo.ipynb](examples/smartcito_training_demo.ipynb) locally or replicate its cells in Kaggle.
2. Prepare the dataset with `training/prepare_dataset.py`.
3. Run `training/lora_training.py` or `training/qlora_training.py`.
4. Confirm the adapter output is written to `output/smartcito-lora/`.
5. Optionally evaluate the adapter before sharing it:

```bash
python training/evaluate_adapters.py --adapter-path output/smartcito-lora
```
...
- `output/smartcito-lora/evaluation_summary.json`
- `output/smartcito-lora/evaluation_report.md`

For offline scoring of saved predictions, run:

```bash
python training/evaluate_adapters.py \
	--dataset datasets/sample_evaluation_data.json \
	--predictions-file datasets/sample_predictions.json
```

## Inference Steps

1. Open [examples/smartcito_inference_demo.ipynb](examples/smartcito_inference_demo.ipynb).
...
4. Use local or merged-local inference to test prompts.
