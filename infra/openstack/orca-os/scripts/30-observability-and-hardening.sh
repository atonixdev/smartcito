...
cat >/etc/ssh/sshd_config.d/10-orca-hardening.conf <<'EOF'
PermitRootLogin no
...
cat >/etc/security/pwquality.conf.d/10-orca.conf <<'EOF'
minlen = 14
dcredit = -1
ucredit = -1
lcredit = -1
...
cat >/etc/sysctl.d/99-orca-security.conf <<'EOF'
...
install -d /opt/orca/bin
cat >/opt/orca/bin/validate_image.sh <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
echo "Orca OS validation passed"
EOF
chmod 0755 /opt/orca/bin/validate_image.sh