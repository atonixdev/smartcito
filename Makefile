PYTHON ?= python3
DATASET ?= datasets/sample_training_data.json
EVAL_DATASET ?= datasets/sample_evaluation_data.json
...
ai-help:
	...
	@printf '%s\n' '                                        Set KAGGLE_PRIVATE=1, KAGGLE_UPDATE=1, or KAGGLE_DRY_RUN=1 as needed'
	@printf '%s\n' '  make ai-full                         Package, prepare, and evaluate in sequence'

ai-package:
	$(PYTHON) training/package_kaggle_bundle.py --output-dir $(KAGGLE_BUNDLE_DIR)

ai-prepare:
	$(PYTHON) training/prepare_dataset.py --input $(DATASET) --output datasets/prepared_smartcito_training_data.jsonl

ai-train-lora:
	$(PYTHON) training/lora_training.py --dataset $(DATASET) --base-model $(BASE_MODEL) --output-dir $(ADAPTER_PATH)

ai-train-qlora:
	$(PYTHON) training/qlora_training.py --dataset $(DATASET) --base-model $(BASE_MODEL) --output-dir $(ADAPTER_PATH)

ai-evaluate:
	$(PYTHON) training/evaluate_adapters.py --dataset $(EVAL_DATASET) --base-model $(BASE_MODEL) --adapter-path $(ADAPTER_PATH) --output $(EVAL_OUTPUT) --markdown-report $(EVAL_REPORT)

ai-report: ai-evaluate

ai-publish-kaggle:
	$(PYTHON) training/publish_kaggle_dataset.py --bundle-dir $(KAGGLE_BUNDLE_DIR) --owner $(KAGGLE_OWNER) --slug $(KAGGLE_SLUG) --title "$(KAGGLE_TITLE)" --license $(KAGGLE_LICENSE) $(KAGGLE_PRIVATE_FLAG) $(KAGGLE_UPDATE_FLAG) $(KAGGLE_DRY_RUN_FLAG)
